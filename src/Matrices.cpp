...
Matrix4 Matrix4::Frustum(float l, float r, float b, float t, float n, float f)
{
    ...
    mat[8]  = (r + l) / (r - l);
    mat[9]  = (t + b) / (t - b);
    mat[10] = ( f >= (float)INFINITY ? EPSILON-1.0f : -(f + n) / (f - n) );
    mat[11] = -1;
    mat[14] = ( f >= (float)INFINITY ? (EPSILON-2.0f) * n : -(2 * f * n) / (f - n) );
    mat[15] = 0;
    return mat;
...
///////////////////////////////////////////////////////////////////////////////
Matrix4 Matrix4::Perspective(float fovY, float aspect, float front, float back)
{
    ...
    // params: left, right, bottom, top, near, far
    return Frustum(-width, width, -height, height, front, back);
}
