...
float wrap( float target, float lower, float upper){
  float dif = upper - lower;

  if(target > lower)
    return lower + (float)fmod((target - lower), dif);

  target = lower + (lower - target);
  float tmp = lower + (float)fmod((target - lower), dif);
  return upper - tmp;
}
...
void Entity3D::setPitch(float degrees)
{
  ...
  //if (this->pitch < -90.0) this->pitch = -90;
  this->pitch = wrap(this->pitch, -180.0f, 180.0f);
#ifdef DEBUG_TRACE_ENTITY
...
void Entity3D::setYaw(float degrees)
{
  this->yaw = degrees;
  // Clamp to sane values
  this->yaw = wrap(this->yaw, -180.0f, 180.0f);
#ifdef DEBUG_TRACE_ENTITY
...
void Entity3D::setRoll(float degrees)
{
  this->roll = degrees;
  // Clamp to sane values
  this->roll = wrap(this->roll, -180.0f, 180.0f);
#ifdef DEBUG_TRACE_ENTITY
